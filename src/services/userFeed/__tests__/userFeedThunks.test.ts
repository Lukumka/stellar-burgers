...
import userFeedReducer, { initialState } from '../userFeedSlice';
...
describe('getUserFeed', () => {
  it('getUserFeed - pending', async () => {
    const action = { type: getUserFeed.pending.type };
    const state = userFeedReducer(initialState, action);
    expect(state.isLoading).toBe(true);
    expect(state.error).toBe(null);
  });
  it('getUserFeed - fulfilled', async () => {
    const mockFeeds = {
      success: true,
      orders: [
        {
    ...
    (getOrdersApi as jest.Mock).mockResolvedValue(mockFeeds);
    const thunk = getUserFeed();
    const result = await thunk(dispatch, getState, {});

    expect(getOrdersApi).toHaveBeenCalled();
    expect(result.type).toBe('userFeed/getUserFeed/fulfilled');
    expect(result.payload).toEqual(mockFeeds);
  });
  it('getUserFeed - rejected ', async () => {
    (getOrdersApi as jest.Mock).mockRejectedValue(
      new Error('failed to get user feed')
    );
    const thunk = getUserFeed();
    const result = await thunk(dispatch, getState, undefined);
    expect(result.type).toBe('userFeed/getUserFeed/rejected');
    expect(result.payload).toBe('Ошибка загрузки заказов пользователя');
  });
});
